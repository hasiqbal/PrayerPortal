...
export async function fetchAdhkar(category?: string): Promise<Dhikr[]> {
  let query = supabase
    .from('adhkar')
    .select(ADHKAR_LIST_COLS)
    .order('prayer_time', { ascending: true })
    .order('group_order', { ascending: true })
    .order('display_order', { ascending: true })
    .order('created_at', { ascending: true });
  if (category) query = query.eq('prayer_time', category);
  const { data, error } = await query;
...
export async function deleteAdhkarByGroup(groupName: string): Promise<void> {
  ...
  if (error) throw new Error(`Failed to delete adhkar entries for group "${groupName}": ${error.message}`);
}
